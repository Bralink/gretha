...
export const VideoPage: NextPage = ({ id }: any) => {
    ...
    useEffect(() => {
        if (ref.current){
          setTimeout(() => {
            ...
            // (ref.current?.plyr as Plyr).on("timeupdate", (e:any) => {
            //     if(ref.current!!.plyr.currentTime >= (ref.current!!?.plyr!!.duration/2)){
            //         setHelpers({
    ...
    //     setTimeout(() => {
    //         console.log(helpers);
    //         timeUpdate(helpers.currentTime);
    //     }, 100);
    // },[helpers]);
               ...
               <Col md={3} className="sectionsContainer">
                    ...
                    <Row className="rowSections" xs={1} md={1} lg={1}>
                        {dataVideo.sections !== undefined ? dataVideo.sections.map((element: SectionVideo) => {
                            return <Col key={element.id} style={{borderBottom: '1px solid lightgray'}}><button className="buttonTopic" onClick={() => timeUpdate((element.minute * 60) + element.second)}>{element.topic}</button></Col>
